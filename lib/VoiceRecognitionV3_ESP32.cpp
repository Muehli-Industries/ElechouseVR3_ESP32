/**
 ******************************************************************************
 * @file      VoiceRecognitionV3_ESP32.cpp
...
VR* VR::instance;
HardwareSerial* VR::serial = &Serial2;
...
/**
    @brief Output HEX-formatted debug
*/
int VR::writehex(uint8_t* buf, uint8_t len) {
    for (int i = 0; i < len; i++) {
        Serial.print(hextab[(buf[i] >> 4) & 0x0F]);
        Serial.print(hextab[buf[i] & 0x0F]);
        Serial.print(" ");
    }
    Serial.println();
    return len;
...
/**
    @brief send data packet with command and data using protocol format.
    @param cmd Command
...
void VR::send_pkt(uint8_t cmd, uint8_t subcmd, uint8_t* buf, uint8_t len) {
    ...
    serial->write(FRAME_HEAD); delay(TXDELAY);
    serial->write(len + 3); delay(TXDELAY);
    serial->write(cmd); delay(TXDELAY);
    serial->write(subcmd); delay(TXDELAY);
    for (uint8_t i = 0; i < len; i++) {
        serial->write(buf[i]); delay(TXDELAY);
    }
    serial->write(FRAME_END); delay(TXDELAY);
}

/**
    @brief send data packet with command and data.
    @param cmd Command
    @param buf Data buffer
    @param len Length of data buffer
*/
void VR::send_pkt(uint8_t cmd, uint8_t* buf, uint8_t len) {
    ...
    serial->write(FRAME_HEAD); delay(TXDELAY);
    serial->write(len + 2); delay(TXDELAY);
    serial->write(cmd); delay(TXDELAY);
    for (uint8_t i = 0; i < len; i++) {
        serial->write(buf[i]); delay(TXDELAY);
    }
...
void VR::send_pkt(uint8_t* buf, uint8_t len) {
    ...
    serial->write(FRAME_HEAD); delay(TXDELAY);
    serial->write(len + 1); delay(TXDELAY);
    for (uint8_t i = 0; i < len; i++) {
        serial->write(buf[i]); delay(TXDELAY);
    }
    serial->write(FRAME_END); delay(TXDELAY);
...
/**
    ...
    @param buf return value buffer.
    @param timeout time in ms for receiving.
    @retval '>0' success, length of packet
            '<0' error
*/
int VR::receive_pkt(uint8_t* buf, uint16_t timeout) {
    int ret = receive(buf, 2, timeout);
    if (ret != 2) return -1;
    if (buf[0] != FRAME_HEAD) return -2;
    if (buf[1] < 2) return -3;
